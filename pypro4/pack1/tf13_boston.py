...
from conda._vendor.tqdm import std
aa = boston_housing.load_data()
# print(aa, type(aa))
(x_train, y_train), (x_test, y_test) = boston_housing.load_data()   # 8:2 로 분리되어 저장됨
...
"""
print('--- concatenate를 사용해보자 ---')
# concatenate : numpy 배열 결합
a = np.array([[1,2],[3,4]])   # 2by2
b = np.array([[5,6],[7,8],[9,10]])    # 3by2
print(np.concatenate((a, b), axis = 0)) # 열기준으로 합친다.
print(np.concatenate((a, b.T), axis = 1)) # 행기준으로 합친다.
print(np.concatenate((a, b), axis = None)) # 첫번째 배열 부터 가로방향 -> 차원 축소

print('~~~ k-fold 를 사용해보자 ~~~')
# 모델 학슴 및 검정 : k-fold 적용 - 비교적 가용 데이터가 적은 경우에 효과적 train만 학습에 참여하기때문에 test는 할 필요가 없다.
k = 4
val_samples = len(x_train) // k # 소수 이하는 버린다.
all_mse_history = [] # 전체 평균 에러를 기억할꺼야
...
for i in range(k):
    ...
    # validation data 를 제외한 나머지는 train으로 쓰면 된다.
