...
model2 = Model(inputs, outputs) # 입력, 출력
# 이하는 방법 1과 동일
...
print('모델 작성방법 3 : sub classing : 동적인 구조 네트워크. 고난이도의 작업에서 활용성이 높음')
x_data = np.array([[1],[2],[3],[4],[5]], dtype=np.float32)
y_data = np.array([11,32,53,64,70], dtype=np.float32)
...
print('모델 작성방법 3-1 : custom layer + sub classing : 동적인 구조 네트워크. 고난이도의 작업에서 활용성이 높음')
...
tf.keras.utils.plot_model(model, 'abc.png') # Layer를 이미지 파일로 만들어준다.